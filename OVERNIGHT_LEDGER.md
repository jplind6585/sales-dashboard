...
- **Remaining Wave 3 (UX+rebrand):** tasks toolbar (3.2), inline task actions (3.3), call-queue outcome (3.5), last-call rows (3.7), queue merge (1.6/3.8), timeline tab (1.3), touch consolidation (1.4/2.10), pursuit/outbound→Supabase (1.7), persistent top-nav from registry + delete shadow /modules (3.4/1.10), apply brand to screens + kill Live pill (5.7) + chart fixes (5.6) + adopt UI kit. Then Wave 4 (coaching/process), 5 (systems+moonshots), Integrations, Big test.
