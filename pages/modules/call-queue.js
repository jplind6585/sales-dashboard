...
export default function CallQueue() {
  ...
  const logTouch = async (accountId, touchType) => {
    try {
      const res = await fetch('/api/sdr/log-touch', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ accountId, touchType }),
      })
      const json = await res.json()
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900 flex items-center gap-2"><Phone className="w-4 h-4 text-emerald-600" /> Call Queue</h1>
          ...
          <div className="flex items-center gap-3">
            <div className="flex rounded-lg border border-gray-200 overflow-hidden text-xs">
              <button onClick={() => setScope('mine')} className={`px-3 py-1.5 ${scope === 'mine' ? 'bg-emerald-600 text-white' : 'text-gray-600'}`}>My accounts</button>
              <button onClick={() => setScope('all')} className={`px-3 py-1.5 ${scope === 'all' ? 'bg-emerald-600 text-white' : 'text-gray-600'}`}>All</button>
            </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-6 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <div className="h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-emerald-500" style={{ width: `${pct}%` }} /></div>
        </div>
              ...
              <div className="p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <button onClick={() => router.push(`/modules/account-pipeline?account=${q.accountId}`)} className="font-medium text-gray-900 hover:text-emerald-700 truncate">{q.name}</button>
                ...
                <span className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${q.channel === 'call' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}`}>
                  {q.channel === 'call' ? <Phone className="w-3 h-3" /> : <Mail className="w-3 h-3" />}{q.channel}
                </span>
                <button onClick={() => draftOpener(q.accountId)} className="text-xs flex items-center gap-1 px-2 py-1.5 border rounded-lg text-violet-600 hover:bg-violet-50">
                ...
                <div className="flex gap-1">
                  {['call', 'email', 'linkedin'].map(t => (
                    <button key={t} onClick={() => logTouch(q.accountId, t)} title={`Log ${t}`} className="text-[11px] px-2 py-1.5 border rounded-lg text-gray-500 hover:bg-gray-50">{t === 'call' ? '📞' : t === 'email' ? '✉️' : 'in'}</button>
                  ))}
                </div>
              </div>
                  ...
                  {draftingId === q.accountId ? <div className="text-sm text-gray-400 flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Drafting opener from {q.name}'s calls…</div> : draftError[q.accountId] && !drafts[q.accountId] ? (
                    ...
                    <>
                      ...
                      <div className="flex gap-2 mt-2">
                        ...
                        <button onClick={() => copyDraft(q.accountId)} className="text-xs flex items-center gap-1 px-2 py-1 border rounded-lg text-gray-600 hover:bg-gray-50">{copiedId === q.accountId ? <><Check className="w-3.5 h-3.5 text-emerald-500" /> Copied</> : <><Copy className="w-3.5 h-3.5" /> Copy</>}</button>
                      </div>
                    </>
                  )}
                </div>
        ...
        <p className="text-xs text-gray-400">Ranked from real pipeline data. Logging a touch needs the <code>sdr_touches</code> migration (see ledger) — until then the counter holds at the server value.</p>
      </main>
    </div>
  )
}
