...
// TRACKING / REPORTING ONLY (CEO rule 2026-07-24): the inactive_* buckets are dead deals that will
// migrate to closed_lost over time, so every metric/report counts them as losses now. NEVER use
// these for navigation, display, ordering, or DB writes — those keep INACTIVE_STAGE_IDS/CLOSED_STAGE_IDS
// so inactive accounts stay browsable and workable until James migrates them.
export const LOST_STAGE_IDS = ['closed_lost', ...INACTIVE_STAGE_IDS]
export const CLOSED_FOR_TRACKING_IDS = ['closed_won', ...LOST_STAGE_IDS]
export const isLostForTracking = (stage) => LOST_STAGE_IDS.includes(stage)
export const isClosedForTracking = (stage) => CLOSED_FOR_TRACKING_IDS.includes(stage)
