...
import { scoreTask, priorityBand } from '../taskPriority'
 ...
 */
export async function getTasks({ ownerId, status, type, accountId, coachedBy, includeDismissed = false } = {}) {
  ...
  let query = supabase
    ...
    .select(`
      *,
      accounts ( id, name, stage, vertical, tier, hubspot_deal_id, deal_value )
    `)
  ...
  // Hide snoozed tasks until they wake (a snooze sets snooze_until, not due_date).
  query = query.or(`snooze_until.is.null,snooze_until.lte.${new Date().toISOString()}`)
  if (ownerId)    query = query.eq('owner_id', ownerId)
  if (coachedBy)  query = query.eq('coached_by', coachedBy)
...
/**
 * Find an existing task by its (source, source_id) — used for idempotent trigger-sourced
 * creation (calendar prep/follow-up, etc.) so the same event can't spawn duplicates
 * across days, devices, or cache-clears. Returns the transformed task or null.
 */
export async function findTaskBySource(source, sourceId) {
  if (!source || !sourceId) return null
  const supabase = getSupabase()
  const { data } = await supabase
    .from('tasks')
    .select(`*, accounts ( id, name, stage, vertical, tier, hubspot_deal_id, deal_value )`)
    .eq('source', source)
...
export async function getTeamTaskSummary() {
  ...
  // Fetch active (non-dismissed) tasks with profile data. Auth is enforced at the API route; this
  // runs on the service-role client, so the owner scoping/visibility must be applied here.
  ...
  for (const task of tasks) {
    ...
    if (task.visible_to_manager === false) continue // rep marked this task private to them
...
export async function createTasks(createdBy, items) {
  ...
  const { data: tasks, error } = await supabase
    ...
    .select(`
      *,
      accounts ( id, name, stage, vertical, tier )
    `)
...
const STAGE_CHECKLISTS = {
  ...
  // Intro → demo
  demo: [
    { title: 'Prepare deck', description: 'Customise the demo deck for this account — pull in their company name, vertical, known pain points, and any "What We Have Heard" bullets from prior calls.', priority: 1 },
    { title: 'Review previous notes and pursuit channel', description: 'Re-read all call notes and the pursuit Slack channel before the demo. Know exactly where things left off.', priority: 1 },
    { title: 'Plan out the ask for next steps', description: 'Decide before the call what the ideal next step is and how you will ask for it at the close of the demo.', priority: 1 },
  ],
...
function transformTaskFromDb(t) {
  if (!t) return null
  const priorityScore = scoreTask({ sourceType: t.source_type, source: t.source, priority: t.priority, dueDate: t.due_date, createdAt: t.created_at })
  return {
    id:               t.id,
    ownerId:          t.owner_id,
    ...
    type:             t.type,
    priority:         t.priority,
    priorityScore,                       // 0-100 holistic score (read-time, fresh)
    priorityBand:     priorityBand(priorityScore), // 'high' | 'med' | 'low' — drives the badge
    ...
    aiDraft:          t.ai_draft || null,
    snoozeUntil:      t.snooze_until || null,
    coachedBy:        t.coached_by || null,
    // Computed helpers
...
function transformTaskToDb(t) {
  ...
  if (t.momentum         !== undefined) db.momentum           = t.momentum
  if (t.trigger          !== undefined) db.trigger            = t.trigger
  if (t.aiDraft          !== undefined) db.ai_draft           = t.aiDraft
  if (t.snoozeUntil      !== undefined) db.snooze_until       = t.snoozeUntil
  return db
}
