...
import { followUpEmailGuide, displayName, stripDashes } from './emailTemplates';
...
// `transcript` (raw call transcript) lets us draft straight from the conversation even when the
// structured analysis is thin ("Analysis unavailable") or the call has no account yet (inbound intro).
export async function generateTaskDraft({ task, account = null, calls = [], transcript = null, repName = 'the rep' }) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return null;

  const { kind, role } = deriveTaskAction(task);
  const today = new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' });
  const signName = displayName(repName);

  // Prefer the raw transcript (richer, always present when a call exists); fall back to analyzed history.
  const transcriptContext = transcript
    ? `\n\nCALL TRANSCRIPT (draft directly from this — speakers are tagged [REP]/[PROSPECT]):\n${String(transcript).slice(0, 16000)}`
    : '';
  const roleInstr = role === 'email-draft'
    ? followUpEmailGuide(signName)
  ...
  const system = [
    ...
    transcriptContext || buildCallContext(calls) || null,
    ``,
    roleInstr,
    ``,
    `TODAY: ${today}`,
  ...
  try {
    ...
    // Client-facing emails must never contain em/en dashes, no matter what the model did.
    const finalContent = role === 'email-draft' ? stripDashes(content) : content.trim();
    return {
      kind,
      role: role || 'generic',
