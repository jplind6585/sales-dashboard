...
import { STAGE_PROBABILITY, ACTIVE_STAGE_ORDER, CLOSED_STAGE_IDS, isClosedForTracking, isLostForTracking } from './constants';
...
export function winRate(accounts = [], { windowDays = 90 } = {}) {
  ...
  for (const a of accounts) {
    if (!isClosedForTracking(a.stage)) continue; // inactive_* count as closed losses (CEO rule 2026-07-24)
    ...
    else lost++; // closed_lost + inactive_sdr_follow_up + inactive_ae_follow_up
  }
