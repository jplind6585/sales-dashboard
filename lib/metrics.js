...
import { STAGE_PROBABILITY, ACTIVE_STAGE_ORDER, CLOSED_STAGE_IDS, isLostForTracking } from './constants';
...
export function accountConfidence(account, { callCount = 0, stakeholders = [] } = {}) {
  ...
  if (isLostForTracking(account?.stage)) return 0; // closed_lost + inactive_* are dead deals → 0
...
export function winRate(accounts = [], { windowDays = 90 } = {}) {
  ...
  for (const a of accounts) {
    // Win rate is over real closed OPPORTUNITIES only — closed_won vs closed_lost. Inactive/dormant
    // deals never got past active pursuit (never reached a close), so they are NOT opportunities and
    // are excluded from win rate (CEO rule 2026-07-25). They still count as dead/not-active elsewhere.
