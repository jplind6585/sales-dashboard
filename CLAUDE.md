...
- **Inactive stages count as `closed_lost` for tracking (CEO rule, 2026-07-24)** — `inactive_sdr_follow_up` + `inactive_ae_follow_up` are dead deals being migrated to `closed_lost` over time. Every metric/report (win rate, loss counts, conversion, active-pipeline denominators, exec snapshots) treats them as losses. But they are NOT migrated yet: navigation (browse filters/dropdowns), display (`STAGE_COLORS`/`STAGE_LABELS`/`STAGE_PROBABILITY`/stage order), and HubSpot sync stage-writes keep them as their own browsable/workable bucket. Canonical sets in `lib/constants.js`: `LOST_STAGE_IDS` / `CLOSED_FOR_TRACKING_IDS` + `isLostForTracking()` / `isClosedForTracking()` — use these for tracking; keep `INACTIVE_STAGE_IDS` / `CLOSED_STAGE_IDS` for nav/display. Folded in `lib/metrics.js` (`winRate`, `accountConfidence`) + pipeline-overview, bottleneck, gong/competitive-analytics, manager/weekly-brief. Impact when applied: all-time win rate 24.4%→4.4%, true active pipeline 1,295→134 accounts.
